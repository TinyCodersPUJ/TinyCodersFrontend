...
@Component({
  selector: 'app-pages-m6',
  templateUrl: './pages-m6.component.html',
  styleUrls: ['./pages-m6.component.css']
})
export class PagesM6Component implements OnInit {
  page = 1;
  lastPage = 6;
  playPage = 5;
  ...
  private scratchURL = 'https://hardware-for-education.github.io/Scratch_For_Education?modulo=modulo6.sb3';
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(p => {
      ...
      this.imgSrc = `assets/img/Modulo6/M6P${this.page}.png`;

      // Anterior / Siguiente
      const prev = Math.max(1, this.page - 1);
      const next = Math.min(this.lastPage, this.page + 1);

      this.prevLink = `/Modulo6/${prev}`;
      this.nextLink = `/Modulo6/${next}`;
    });
