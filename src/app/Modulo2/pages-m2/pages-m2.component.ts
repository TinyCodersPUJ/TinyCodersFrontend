...
@Component({
  selector: 'app-pages-m2',
  templateUrl: './pages-m2.component.html',
  styleUrls: ['./pages-m2.component.css']
})
export class PagesM2Component implements OnInit {
  page = 1;
  lastPage = 4;
  playPage = 3;
  ...
  private scratchURL = 'https://hardware-for-education.github.io/Scratch_For_Education?modulo=modulo2.sb3';
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(p => {
      ...
      this.imgSrc = `assets/img/Modulo2/M2P${this.page}.png`;

      // Anterior / Siguiente
      const prev = Math.max(1, this.page - 1);
      const next = Math.min(this.lastPage, this.page + 1);

      this.prevLink = `/Modulo2/${prev}`;
      this.nextLink = `/Modulo2/${next}`;
    });
