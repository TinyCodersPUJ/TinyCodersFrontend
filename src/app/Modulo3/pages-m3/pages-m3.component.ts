...
@Component({
  selector: 'app-pages-m3',
  templateUrl: './pages-m3.component.html',
  styleUrls: ['./pages-m3.component.css']
})
export class PagesM3Component implements OnInit {
  page = 1;
  lastPage = 5;
  playPage = 4;
  ...
  private scratchURL = 'https://hardware-for-education.github.io/Scratch_For_Education?modulo=modulo3.sb3';
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(p => {
      ...
      this.imgSrc = `assets/img/Modulo3/M3P${this.page}.png`;

      // Anterior / Siguiente
      const prev = Math.max(1, this.page - 1);
      const next = Math.min(this.lastPage, this.page + 1);

      this.prevLink = `/Modulo3/${prev}`;
      this.nextLink = `/Modulo3/${next}`;
    });
