...
@Component({
  selector: 'app-pages-m4',
  templateUrl: './pages-m4.component.html',
  styleUrls: ['./pages-m4.component.css']
})
export class PagesM4Component implements OnInit {
  page = 1;
  lastPage = 4;
  playPage = 3;
  ...
  private scratchURL = 'https://hardware-for-education.github.io/Scratch_For_Education?modulo=modulo4.sb3';
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(p => {
      ...
      this.imgSrc = `assets/img/Modulo4/M4P${this.page}.png`;

      // Anterior / Siguiente
      const prev = Math.max(1, this.page - 1);
      const next = Math.min(this.lastPage, this.page + 1);

      this.prevLink = `/Modulo4/${prev}`;
      this.nextLink = `/Modulo4/${next}`;
    });
